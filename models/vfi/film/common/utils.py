...
def warp(image: torch.Tensor, flow: torch.Tensor) -> torch.Tensor:
    ...
    h, w = int(flow.shape[2]), int(flow.shape[3])
    img_h, img_w = int(image.shape[2]), int(image.shape[3])
    ls1 = 1 - 1 / w
    ls2 = 1 - 1 / h
    normalized_flow2 = flow.permute(0, 2, 3, 1) / torch.tensor(
        [w * .5, h * .5], dtype=dtype, device=device)[None, None, None]
    normalized_flow2 = torch.stack([
        torch.linspace(-ls1, ls1, w, dtype=dtype, device=device)[None, None, :] - normalized_flow2[..., 1],
        torch.linspace(-ls2, ls2, h, dtype=dtype, device=device)[None, :, None] - normalized_flow2[..., 0],
    ], dim=3)

    warped = F.grid_sample(image, normalized_flow2,
                           mode='bilinear', padding_mode='border', align_corners=False)
    return warped.reshape(image.shape[0], image.shape[1], img_h, img_w)
