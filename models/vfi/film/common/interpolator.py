...
class Interpolator(nn.Module):
    ...
    def shuffle_images(self, x0, x1):
        return [
            utils.build_image_pyramid(x0, self.pyramid_levels),
            utils.build_image_pyramid(x1, self.pyramid_levels)
        ]
    ...
    def debug_forward(self, x0, x1, batch_dt) -> Dict[str, List[torch.Tensor]]:
        ...
        forward_flow_pyramid = utils.flow_pyramid_synthesis(forward_residual_flow_pyramid)[:self.fusion_pyramid_levels]

        backward_flow_pyramid = utils.flow_pyramid_synthesis(backward_residual_flow_pyramid)[:self.fusion_pyramid_levels]
        ...
        backward_flow = utils.multiply_pyramid(backward_flow_pyramid, batch_dt)
        forward_flow = utils.multiply_pyramid(forward_flow_pyramid, 1 - batch_dt)

        pyramids_to_warp = [
            utils.concatenate_pyramids(image_pyramids[0][:self.fusion_pyramid_levels],
                                      feature_pyramids[0][:self.fusion_pyramid_levels]),
            utils.concatenate_pyramids(image_pyramids[1][:self.fusion_pyramid_levels],
        ...
        aligned_pyramid = utils.concatenate_pyramids(forward_warped_pyramid,
