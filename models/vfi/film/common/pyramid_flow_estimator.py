...
class FlowEstimator(nn.Module):
    ...
    def __init__(self, in_channels: int, num_convs: int, num_filters: int):
        ...
        for i in range(num_convs):
            self._convs.append(utils.Conv2d(in_channels=in_channels, out_channels=num_filters, size=3))
            in_channels = num_filters
        self._convs.append(utils.Conv2d(in_channels, num_filters // 2, size=1))
        ...
        self._convs.append(utils.Conv2d(in_channels, 2, size=1, activation=None))
...
class PyramidFlowEstimator(nn.Module):
    ...
    def forward(self, feature_pyramid_a: List[torch.Tensor],
        ...
        utils.flow_pyramid_synthesis can be used to convert the residual flow
        ...
        for i in range(levels - 2, len(self._predictors) - 1, -1):
            ...
            warped = utils.warp(feature_pyramid_b[i], v)
